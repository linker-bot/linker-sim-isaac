...
from linkerbot_sim.tiled.scene.paths import env_origins, env_root_paths
...
class DebugTiledSceneRuntime:
    ...
    robot_names: tuple[str, ...] = (DEBUG_TRAJECTORY_ROBOT,)
    step: int = 0
    ...
    def create(
        ...
        *,
        ...
        tcp_frame_name: str,
        ik_solver: DebugBatchIKBackend | None,
    ...
    def step_action(
        ...
        *,
        ...
        ticks = _action_decimation(
            action,
            default_decimation=self.adapter.default_decimation,
        ...
        previous_adapter_target = (
            None
            if self.adapter.last_target is None
            else self.adapter.last_target.copy()
        )
        if action.kind == "ee_linear_path":
            path = self.adapter.linear_path_to_joint_trajectory(
                action,
                steps=int(ik_steps),
            ...
            trajectory = path.joint_positions
            info = path.info
        else:
        ...
        info = dict(info)
        info["failed_env_ids"] = np.asarray(failed_env_ids, dtype=int)
        if self.adapter.failure_policy == "reject_request" and failed_env_ids:
            self.adapter.last_target = previous_adapter_target
            raise TiledIKRequestRejected(failed_env_ids)
        for tick_target in trajectory:
            self.current_positions = tick_target.copy()
            self.step += 1
            self.episode_steps[:] += 1
        self._update_debug_tcp_state(
            action,
            env_ids=selected,
            info=info,
            steps=(int(ik_steps) if ik_steps is not None else None),
        )
        response = {
            "event": "step",
            "accepted": True,
            ...
            "episode_steps": self.episode_steps.tolist(),
            "joint_positions": self.current_positions.tolist(),
            "info": _jsonable_mapping(info),
        }
        if action.kind == "ee_linear_path":
            response["duration_s"] = float(trajectory.shape[0]) * self.physics_dt
            response["sample_dt_s"] = float(
                self.physics_dt if action.sample_dt_s is None else action.sample_dt_s
            )
    ...
    def get_state(
        ...
        *,
        ...
        payload = {
            "robots": {
                DEBUG_TRAJECTORY_ROBOT: {
                    "joint_names": [
                        f"joint_{index}" for index in range(self.adapter.command_dim)
                    ],
                    "joint_positions": self.current_positions[selected].tolist(),
                    "joint_velocities": np.zeros_like(
                        self.current_positions[selected]
                    ).tolist(),
                    "tcp_positions_world": self.current_tcp_positions[
                        selected
                    ].tolist(),
    ...
    def get_snapshot(self, *, env_id: int) -> dict[str, object]:
        ...
        snapshot = get_tiled_scene_snapshot(self, env_id=int(env_id))
        return {
            "event": "snapshot",
            "accepted": True,
            "backend": "debug",
    ...
    def step_trajectory(
        ...
        *,
        ...
        planner_results, planner_loaded, load_rejected = self._collect_planner_results()
        selected = _normalize_env_ids(env_ids, self.config.num_envs)
    ...
    def planner_status(self, *, wait_timeout_s: float = 0.0) -> dict[str, object]:
        ...
        ready, loaded, load_rejected = self._collect_planner_results(
    ...
    def _update_debug_tcp_state(
        ...
        *,
        ...
        if action.kind == "ee_delta_pose":
            ...
            return
        if action.kind == "ee_linear_path":
            if info is None or steps is None:
                raise ValueError("ee_linear_path debug update requires path info")
            completed = np.asarray(info["ik_completed_steps"], dtype=float)
            alpha = completed / float(steps)
            if action.interpolation == "smoothstep":
                alpha = alpha * alpha * (3.0 - 2.0 * alpha)
            start_positions = self.current_tcp_positions.copy()
            if action.target_position is not None:
                target_positions = _batched_values(
                ...
                target_positions = start_positions + _batched_values(
                    offset_values,
                    self.config.num_envs,
                    3,
                    "target_offset",
            ...
            self.current_tcp_positions[env_ids, :] = (
                start_positions + alpha[:, None] * (target_positions - start_positions)
            )[env_ids, :]
            if (
                action.orientation_mode == "target"
                and action.target_orientation_wxyz is not None
            ):
                target_orientations = _normalize_quaternions(
                    _batched_values(
                ...
                self.current_tcp_orientations_wxyz[env_ids, :] = (
                    _slerp_quaternions_wxyz(
                        self.current_tcp_orientations_wxyz,
                        target_orientations,
                        alpha,
                    )[env_ids, :]
                )
    ...
    def _collect_planner_results(
        ...
        *,
        ...
        loaded, rejected = load_ready_planning_results(self.trajectory_buffer, results)
