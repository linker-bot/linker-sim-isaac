...
def test_sample_cartesian_pose_line_interpolates_position_and_orientation() -> None:
    ...
    np.testing.assert_allclose([waypoint.time_s for waypoint in waypoints], [0.0, 0.5, 1.0])
    np.testing.assert_allclose([waypoint.position[2] for waypoint in waypoints], [0.0, 0.5, 1.0])
    np.testing.assert_allclose(waypoints[1].orientation, [np.sqrt(0.5), 0.0, 0.0, np.sqrt(0.5)])
    np.testing.assert_allclose(waypoints[2].orientation, [0.0, 0.0, 0.0, 1.0], atol=1.0e-12)
