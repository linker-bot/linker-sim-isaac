...
from linkerbot_sim.mirror.reset_runtime import reset_mirror_scene
...
def _scene() -> MirrorSceneSettings:
    """构造 reset 合同所需的最小 typed Mirror scene。"""

    return MirrorSceneSettings(
        scene_id="reset_test",
        description="Mirror reset transaction fixture",
        gravity_z=-3.0,
        add_ground=False,
        ground_height=0.0,
...
def test_reset_mirror_scene_restores_all_registered_robots() -> None:
    fixture = _make_runtime()

    result = reset_mirror_scene(
        fixture.runtime,
...
def test_reset_skips_unsupported_newton_per_link_gravity() -> None:
    fixture = _make_runtime()
    articulation = fixture.robots[0].prepared.articulation
    articulation.supports_per_link_gravity = False
    articulation.physics_backend = "newton"

    with pytest.warns(
        PhysicsCompatibilityWarning,
        match="robot.physics.gravity",
    ):
        reset_mirror_scene(
            fixture.runtime,
            robot_root_pose_applier=fixture.root_pose_applier,
    ...
    assert articulation.gravity_disabled is False
    np.testing.assert_allclose(articulation.velocities, [0.0, 0.0])


def test_reset_preserves_projected_newton_gravcomp_without_runtime_setter() -> None:
    fixture = _make_runtime()
    prepared = fixture.robots[0].prepared
    articulation = prepared.articulation
    articulation.supports_per_link_gravity = False
    articulation.physics_backend = "newton"
    prepared.gravity_counts = {
        "enabled": 0,
        "disabled": 2,
        "newton_gravcomp": 2,
    }
...
def test_newton_runtime_reset_uses_manager_state_without_usd_pose_writes() -> None:
    fixture = _make_runtime()
    gravity_calls: list[float] = []
    fixture.runtime.session.physics_runtime = SimpleNamespace(
        backend="newton",
        execution="cuda",
        set_gravity=lambda value: gravity_calls.append(float(value)),
    )

    reset_mirror_scene(
        fixture.runtime,
        robot_root_pose_applier=lambda *_args: pytest.fail(
            "Newton reset must not author robot USD poses"
        ),
        object_root_pose_applier=lambda *_args: pytest.fail(
            "Newton reset must not author object USD poses"
        ),
        root_pose_reader=lambda *_args: pytest.fail(
            "Newton reset must not read stale USD poses"
        ),
    )

    assert fixture.world.reset_count == 1
    assert fixture.root_pose_calls == []
    assert gravity_calls == [-3.0]
    for robot in fixture.robots.values():
        assert robot.prepared.joint_controller.configure_count == 1
...
def test_mirror_reset_rejects_fatal_runtime_before_accessing_session() -> None:
    runtime = SimpleNamespace(fatal_error="snapshot rollback failed")

    with pytest.raises(RuntimeMutationRejected, match="requires rebuild"):
        reset_mirror_scene(runtime)
