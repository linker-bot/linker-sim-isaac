...
本文说明如何在当前 `env_isaaclab/` 环境里预览仓库中的 `.usd` / `.usda` 资产。
默认流程是先启动 Isaac Sim GUI，然后通过 `File -> Open` 打开资产文件。
...
```bash
env_isaaclab/bin/isaacsim isaacsim.exp.full
```
...
```bash
env_isaaclab/bin/isaacsim isaacsim.exp.full --exec "open_stage.py assets/rigid_env_objects/TblockV1_default/TblockV1_default.usda"
```

注意：`env_isaaclab/bin/isaacsim` 的第一个非选项参数是 Isaac Sim/Kit 的 experience
配置（`.kit`），不是 USD 文件。直接运行
`env_isaaclab/bin/isaacsim <asset.usda>` 会把 `<asset.usda>` 当作 `.kit` 去查找，
...
`env_isaaclab/bin/isaacsim isaacsim.exp.full --exec "open_stage.py <asset_path>"`。
...
```bash
env_isaaclab/bin/python -c "from pxr import Usd"
```
