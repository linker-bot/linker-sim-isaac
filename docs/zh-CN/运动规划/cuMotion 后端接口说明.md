...
- TCP/frame：必须存在于 cuMotion 加载的 URDF/XRDF 机器人描述中。自定义 TCP 在 robot YAML
  的 `cumotion.custom_tcps` 中声明，并在创建 context 前写入派生 URDF。
...
| `default_tcp_frame` | `str \| None` | 默认 TCP frame；为空时回退到 `flange_frame` |
| `custom_tcp_frames` | `tuple[TcpFrame, ...]` | 从 YAML `custom_tcps` 解析出的 fixed TCP frame，创建 context 前会写入派生 URDF |
...
位置：`configs/robots/*.yaml`

用途：在 robot profile 里声明可供 cuMotion 使用的 TCP frame。每个自定义 TCP 是一个 fixed
link/joint，默认相对 `cumotion.flange_frame`；也可以用 `parent_frame` 指向其它已存在 link。
运行时 JSON 的 `tcp_frame_name` 只能选择这些已经进入 context 的 frame，不能携带临时 TCP 位姿。
...
- `default_tcp_frame` 为空时回退到 `flange_frame`。
- `custom_tcps` 支持 mapping 或 list；mapping 形式以 key 作为 frame name。
- `xyz` 单位 m，`rpy` 单位 rad，缺省均为零。
- frame name 不能重复，也不能和基础 URDF 已有 link 重名。
- `parent_frame` 缺省为 `flange_frame`；如果没有 `flange_frame`，则必须显式提供。
- 双臂融合时会合并左右 robot profile 的 `custom_tcps`，frame 名必须全局唯一。

### `CuMotionContext` materialize

位置：`src/linkerbot_sim/backends/cumotion/context.py`

`CuMotionContext(config)` 会先调用 `materialize_cumotion_config(config)`：如果配置里有
`custom_tcp_frames`，就把这些 fixed link 写入 `.cache/cumotion/*_custom_tcps_<hash>.urdf`，
再用派生 URDF 加载 cuMotion。hash 由基础 URDF 路径、基础 URDF 内容和 TCP 定义决定，因此同一
配置会稳定复用同一个派生文件，基础 URDF 改动后也会生成新的缓存文件。

frame 解析统一走 `resolve_tcp_frame_name(...)`，优先级为：

1. 请求或 JSON 中显式传入的 `tcp_frame_name`。
2. 单臂/双臂/tiled 调用方传入的默认 frame。
3. `CuMotionConfig.default_tcp_frame`。
4. `CuMotionConfig.flange_frame`。

解析结果会用 `validate_cumotion_frame(...)` 校验，要求 frame 非空且已经在当前 context 中存在。

### `write_tcp_urdf(...)`

位置：`src/linkerbot_sim/backends/cumotion/tcp_urdf_builder.py`

用途：复制基础 URDF，并在指定 parent frame 下追加一个 fixed TCP link/joint。它是底层 URDF 写入工具，通常由 `materialize_cumotion_config(...)` 调用；只有测试、诊断或离线生成 URDF 时才需要直接使用。
...
`trajectory_generation.solver_params` 暴露到 context 和动作配置。
