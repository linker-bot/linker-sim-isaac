...
本文说明客户脚本如何选择 TCP frame 并用 Python 参数描述运动。TCP frame 的几何定义属于
robot YAML；运动目标、阶段顺序和实验参数仍留在 Python/JSON 请求里。
...
- `tcp_frame_name` 可选时，单臂/双臂/tiled 执行器会使用 robot YAML 中的默认 TCP。
...
TCP frame 需要先在 robot YAML 的 `cumotion.custom_tcps` 中声明，默认相对
`cumotion.flange_frame`；运行时只通过 `tcp_frame_name` 选择这些已加载 frame，不能再临时注入
TCP 位姿：
...
双臂场景会从左右 robot profile 合并各自的 `custom_tcps`，所以 frame 名需要全局唯一，例如
`AR5V2_L_pinch_tcp` 和 `AR5V2_R_pinch_tcp`。
...
这套 overlay 适用于本文后续 arm/cuMotion motion：绝对 IK、IK offset、绝对 C-space goal、C-space delta、TCP line、TCP arc、C-space waypoint、Composite path 和高级 `CumotionMoveSpec`。

## Absolute IK Pose

绝对 IK pose 表示“让 TCP 到达指定的绝对位置，可选约束目标姿态”。

Python 层可以用高级 `CumotionMoveSpec + IKRequest` 表达；交互协议中对应 `ik_pose` 消息。
...
`IkOffsetMoveSpec` 也支持按模式处理姿态。`orientation_mode="current"` 保持当前 TCP 姿态，`"target"` 使用 `target_orientation`，`"none"` 表示只约束位置：
...
当前可以用高级 `CumotionMoveSpec + MotionRequest(goal_q=...)` 表达完整双臂 C-space 目标；客户脚本层也可以直接使用 `CSpaceGoalPlanMoveSpec`：

```python
CSpaceGoalPlanMoveSpec(
    side="right",
    tcp_frame_name="AR5V2_R_pinch_tcp",
