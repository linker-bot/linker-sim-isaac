...
单臂入口对应打印 `SINGLE_ARM_INTERACTIVE_READY`。此时 Isaac 进程保持运行，外部可以继续发送运动消息。双臂入口的 `--hold` 会让窗口和仿真循环在 stdin EOF 后继续保持当前姿态；单臂入口空闲时会自动刷新保持命令，用于维持 GUI/Foxglove 状态更新。

## Tiled Envs 交互入口

tiled envs 使用独立脚本，不复用旧双臂交互执行队列。它会启动真实 Isaac tiled scene，通过 batched `Articulation` view 对 selected robot 写整批关节目标。它的定位是同步 command-step 调试、episode 控制、离线轨迹回放和外部 async planner 调度；支持把旧 `cspace_goal` / `cspace_delta` / `task_space_line` / `task_space_arc` / `specified_path` 和 `moves` 队列转换成 tiled async plan，但不支持旧 runtime 的 `cancel_current`/`estop` 或每个 env 不同步数推进。
...
async planner 默认使用 `--planner-backend linear`，只支持关节空间目标和关节 MoveSpec 队列。需要 task-space line/arc 或 specified path 时，用 `--planner-backend cumotion` 启动；ready result 仍通过 `planner_status` 或 `step_trajectory` 载入 trajectory buffer。
