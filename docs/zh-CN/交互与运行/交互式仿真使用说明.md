...
```json
{"type":"get_snapshot","id":"snap-1"}
```

返回中的 `snapshot` 可用于同构 runtime restore：
...
跨 role 写入时需要显式声明映射。例如把 dual snapshot 的右臂写到 single runtime：
...
snapshot 请求由 transport 线程放入队列，仿真主线程在安全边界读取或写回状态。默认 `strict:true`，robot profile、asset fingerprint、joint names 或 dynamic-chain body names 不兼容时会拒绝。snapshot restore 不会调用全局 `world.reset()`。
