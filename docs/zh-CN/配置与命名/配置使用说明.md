...
- `env.ground_height` 是 Isaac 默认地面的 z 高度，单位 m；只有 `env.add_ground: true` 时生效。
...
- 如需覆盖输出路径或采样间隔，复制一个 logging profile 并用 `--logging-profile <name>` 选择。
...
5. 运行 `PYTHONPATH=src python -m pytest tests/test_env_profile_directory.py tests/test_controller_configs.py tests/test_robot_loader_import_config.py -q` 做配置链路检查；单臂场景可用单臂交互入口做导入检查，双臂场景可用 `scripts/dual_arm_interactive.py --env my_scene --hold` 做交互 runtime 导入检查。
...
双臂 motion 语义测试不启动 Isaac，适合快速检查左右 robot profile、TCP 和 planner 数据结构：

```bash
PYTHONPATH=src python -m pytest tests/test_dual_arm_motion_test.py -q
```

单臂交互入口会启动 Isaac、导入对象和机器人：
