...
def runtime_objects_from_env_config(
    ...
    instances = object_scene_instances_from_env_config(env_config)
    profiles = {
        instance.object_profile: ObjectProfileConfig.from_profile(
            instance.object_profile
        )
        for instance in instances
    }
    return tuple(
        _runtime_object_from_scene_instance(item, profiles[item.object_profile])
...
def _runtime_object_from_scene_instance(
...
def _add_rigid_object(
    ...
    return RuntimeObjectHandle(
        ...
        config=rigid_config,
        model=added,
        state_summary=config.profile.state_summary,
    )
...
def _placed_capsule_rope_config(
    ...
    return replace(
        ...
        prim_path=runtime_config.prim_path,
