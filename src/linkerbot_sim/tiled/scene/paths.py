"""tiled scene 的 USD path 和 env origin 辅助函数。

robot/object profile 使用单场景路径，例如 ``/World/Robot`` 或 ``/World/TBlock``。tiled
runtime 在加载时通过本模块把这些路径映射到目标 env root，同时保持 profile 对其它 runtime
仍可直接使用。
...
def env_origins(config: TiledEnvConfig) -> np.ndarray:
    ...
    origins = np.repeat(
        np.asarray(config.layout.origin_xyz, dtype=float).reshape(1, 3),
        config.num_envs,
        axis=0,
    )
