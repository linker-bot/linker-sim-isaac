"""Mirror timeline tick 换算、可执行 segment/track 与 direct request 构造器。"""
...
def make_effort_segment(
    *,
    joint_names: Sequence[str],
    hold_positions: Sequence[float],
    joint_efforts: Sequence[float],
    duration_s: float,
    physics_dt: float,
    start_tick: int = 0,
    phase: str = "joint_effort",
    metadata: Mapping[str, object] | None = None,
) -> MotionSegment:
    """Build a constant direct-effort segment on the physics tick grid."""

    names = tuple(str(name) for name in joint_names)
    positions = np.asarray(hold_positions, dtype=float).reshape(-1)
    efforts = np.asarray(joint_efforts, dtype=float).reshape(-1)
    if positions.shape != (len(names),) or efforts.shape != (len(names),):
        raise ValueError("effort segment positions/efforts must match joint_names")
    if not np.all(np.isfinite(positions)) or not np.all(np.isfinite(efforts)):
        raise ValueError("effort segment positions/efforts must be finite")
    ticks = duration_to_ticks(duration_s, physics_dt)
    return MotionSegment(
        joint_names=names,
        positions=np.repeat(positions[None, :], ticks, axis=0),
        velocities=np.zeros((ticks, len(names)), dtype=float),
        efforts=np.repeat(efforts[None, :], ticks, axis=0),
        start_tick=start_tick,
...
def _expand_full_command_unit(
    ...
    split = _split_full_joint_value(
        (
            segment.joint_efforts
            if segment.kind == "joint_effort"
            else segment.joint_positions
        ),
    ...
    value_field = (
        "joint_efforts" if segment.kind == "joint_effort" else "joint_positions"
    )
    return RobotMotionUnitRequest(
        group_tracks=(
            JointGroupTrackRequest(
                "arm", (replace(segment, **{value_field: arm_value}),)
            ),
            JointGroupTrackRequest(
                "hand", (replace(segment, **{value_field: hand_value}),)
            ),
        )
    )
