"""把 Mirror 主线程采样接入 Foxglove realtime state stream。
...
class InteractiveStateStreamConfig:
    ...
    def enabled(self) -> bool:
        ...
        has_modality = (
            ...
            or self.include_hybrid_control
        )
        has_output = (
...
def start_interactive_state_stream(
    ...
    *,
    ...
    if config is None or not config.enabled():
        return None
    if not _looks_like_mirror_resources(runtime):
        raise ValueError("interactive state stream requires MirrorSceneResources")
    ...
    _print_status(
        ...
        f"include_hybrid_control={config.include_hybrid_control} "
...
def _state_observer_for_runtime(
    ...
    *,
    ...
    """为 canonical MirrorSceneResources 创建主线程状态 observer。"""
    ...
    sampler_kwargs = {
        ...
        "include_hybrid_control": config.include_hybrid_control,
        "hybrid_diagnostics_provider": (
            getattr(runtime, "hybrid_diagnostics", None)
            if config.include_hybrid_control
            else None
        ),
    }
...
def _looks_like_mirror_resources(runtime: object) -> bool:
    """用最小结构检查区分 MirrorSceneResources，避免 telemetry 层反向导入具体 runtime 类型。"""
        ...
        if config.foxglove_live_port is not None:
            sinks.append(
                FoxgloveStateSink.open_live(
                    ...
                    publish_hybrid_control=config.include_hybrid_control,
