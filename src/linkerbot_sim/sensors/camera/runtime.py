"""Isaac sensor camera 的创建、intrinsics 写入与延迟初始化 helpers。

Isaac/Omni import 延迟到真正创建 Camera 时执行，使配置解析和普通单元测试不依赖
Isaac Sim。
"""
...
def create_sensor_camera_runtimes(
    *,
    ...
    # A non-None selector means tiled expansion was skipped. Refuse to create
    # an accidentally unscoped camera instead of silently ignoring env_ids.
    sensors.validate_single_scene_camera_scope()
...
def _apply_camera_intrinsics(*, camera: object, settings: SensorCameraSettings) -> None:
    """把显式 pinhole intrinsics 写入 Isaac Camera wrapper。"""
...
def _apply_aperture_intrinsics(
    *,
    ...
    """设置 focal/aperture ratio，使 Isaac native intrinsics 与 pixel 参数一致。"""
...
def _required_camera_method(
    ...
    """读取显式 intrinsics 所需的 Camera method，缺失时给出配置路径错误。"""
...
def _define_camera_prim_with_square_pixels(
    *, stage: object, settings: SensorCameraSettings
) -> None:
    """预先创建 aperture 比例正确的 Camera，避免 wrapper 构造时再修正。"""

    from pxr import UsdGeom

    existing = stage.GetPrimAtPath(settings.prim_path)
    if existing is not None and _prim_is_valid(existing):
        return
    camera = UsdGeom.Camera.Define(stage, settings.prim_path)
    ...
    camera.GetVerticalApertureAttr().Set(
        float(horizontal_aperture) * float(height) / float(width)
    )
