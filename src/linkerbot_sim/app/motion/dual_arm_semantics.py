...
- flange frame 已经是单臂 robot profile 的机器人资源语义。

因此 selected-side 规划所需的左右 C-space 分区可以直接从各自 XRDF 推导，
临时 TCP link 的 parent frame 可以直接从各自 robot profile 的
``cumotion.flange_frame`` 读取。
"""
...
class DualArmRobotSemantics:
    ...
    ``left_flange_frame`` / ``right_flange_frame`` 是创建脚本侧临时 TCP frame
    时绑定到的父 frame。TCP 的名字和偏移仍由调用方传入的 ``DualArmTcpSpec``
    决定，这里只提供“这个 TCP 应挂在哪个机器人 frame 上”的机器人资源语义。
    """
...
def dual_arm_semantics_from_robot_configs(
    ...
    - ``cumotion.flange_frame``：绑定调用方 TCP frame 的父 frame。
...
def _side_robot_flange_frame(robot_config: Mapping[str, object], side: str) -> str:
    ...
    ``DualArmTcpSpec`` 描述脚本希望使用的 TCP frame 名和偏移，但临时 TCP link
    需要挂到机器人已有 frame 上。这个父 frame 是机器人资源语义，应该随 robot
    profile 走，而不是随动作脚本或双臂 profile 另存一份。
    """
