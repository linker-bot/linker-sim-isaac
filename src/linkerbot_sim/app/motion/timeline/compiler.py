"""把多机器人逻辑请求原子编译为可执行整数 tick timeline。
...
class TimelinePlanningSession:
    ...
    def __init__(
        ...
        *,
        ...
        self.physics_dt = float(runtime.world.get_physics_dt())
        ...
        *,
        ...
        if request.kind == "hold":
            return HoldSegment(
                ...
                positions=current,
                duration_ticks=duration_to_ticks(request.duration_s, self.physics_dt),
                phase=phase,
        ...
        if self.planner_backend == "linear":
            if request.kind not in {"plan_cspace_goal", "plan_cspace_delta"}:
                raise RuntimeError(
                    ...
                    "plan_cspace_delta in single_scene mode"
                )
