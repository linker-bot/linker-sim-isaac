"""多机器人 timeline 的主线程控制写入与 world execution loop。"""
...
from linkerbot_sim.app.motion.timeline.model import RobotTimeline
...
def execute_robot_timeline(
    ...
    *,
    ...
    world = _single_scene_world(runtime)
    ...
    states = {
        track.robot_id: _ExecutionState.from_robot(
            _single_scene_robot(runtime, track.robot_id, track.robot_label)
        )
    ...
    try:
        for tick in range(int(timeline.duration_ticks)):
            ...
            world.step(render=_render_enabled(runtime, states.values()))
            sample_step = step
            step += 1
        ...
        # 中断和异常也必须清零速度，避免 PhysX 延续上一 tick 的速度目标。
        for state in states.values():
            set_velocities = getattr(state.articulation, "set_joint_velocities", None)
            if callable(set_velocities):
                set_velocities(np.zeros(state.num_dof, dtype=float))
    return step
...
class _ExecutionState:
    ...
    def from_robot(cls, robot: object) -> "_ExecutionState":
        ...
        full = np.asarray(articulation.get_joint_positions(), dtype=float).reshape(-1)
...
def _single_scene_world(runtime: object):
    """兼容 SingleSceneRuntime 与轻量测试 runtime，解析其唯一 simulation world。"""
    ...
    return runtime.simulation_world


def _single_scene_robot(runtime: object, robot_id: int, label: str | None):
...
def _render_enabled(runtime: object, states: Sequence[_ExecutionState]) -> bool:
    """camera output 或任一 robot execution 请求 render 时启用 World render。"""

    if getattr(runtime, "camera_output", None) is not None:
        return True
    return any(
        bool(getattr(state.execution, "render_enabled", False)) for state in states
    )
...
def _observe_once(runtime: object, states, *, step: int, phase: str) -> None:
    ...
    if callable(observe_camera):
        observe_camera(_single_scene_world(runtime), step=step, phase=phase)
