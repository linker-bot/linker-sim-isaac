"""普通 SingleSceneRuntime 的 canonical JSON 交互协议。

本模块是 transport 与仿真主线程之间的纯数据边界：stdin、TCP JSONL 和 WebSocket 收到的
mapping 都先在这里转换为 ``InteractiveMotionCommand``。解析过程不导入 Isaac 或 cuRobo，
也不读取 articulation 状态，因此可以在网络线程安全执行。
...
class InteractiveMotionCommand:
    ...
    这是 transport 与主线程之间的不可变 DTO。snapshot 字段只在
    ``get_snapshot``/``set_snapshot`` 中使用；它们复用 motion transport，但实际执行
    会交给仿真主循环中的 snapshot adapter。各 kind 未使用的字段保持缺省值，消费者应先
    根据 ``kind`` 分支，不能通过某个可选字段是否为空来推断命令类型。
    """
    ...
    snapshot_label_map: Mapping[str, str] | None = None
    snapshot_strict: bool = True
    timeline: RobotTimelineRequest | None = None
...
def parse_interactive_motion_message(
    ...
    *,
    ...
    """解析一条 canonical robot-ID 命令，不执行任何 runtime 副作用。

    完整 timeline 与单 segment 简写最终都会归一化为 ``RobotTimelineRequest``；控制命令
    则只保存经过类型校验的字段，留给 transport/主线程处理。传入 mapping 不会被修改，
    parser 也不会触发队列 admission 或 runtime 状态变更。
    """
    ...
    } or (command_type == "hold" and "robot_id" in message):
    ...
    if command_type == "reset":
        _reject_unknown_fields(
            message,
            {"type", "id", "clear_queue", "hold_after_reset"},
            "reset",
        )
        return InteractiveMotionCommand(
            kind="reset",
            reset_id=_optional_str(message, "id", label="reset.id"),
            reset_clear_queue=_optional_bool(
                message,
                "clear_queue",
                default=True,
                label="reset.clear_queue",
            ),
            reset_hold_after_reset=_optional_bool(
                message,
                "hold_after_reset",
                default=True,
                label="reset.hold_after_reset",
            ),
        )
    if command_type == "get_snapshot":
        _reject_unknown_fields(message, {"type", "id"}, "get_snapshot")
        # SingleSceneRuntime 没有 tiled env 维度，因此 get_snapshot 不需要 env_id。
        return InteractiveMotionCommand(
            kind="get_snapshot",
            command_id=_optional_str(message, "id", label="get_snapshot.id"),
        )
    if command_type == "set_snapshot":
        _reject_unknown_fields(
            message,
            {"type", "id", "snapshot", "label_map", "strict"},
            "set_snapshot",
        )
        snapshot = message.get("snapshot")
        if not isinstance(snapshot, Mapping):
            raise ValueError("set_snapshot.snapshot must be a JSON object")
        # label_map/strict 直接保留到 command，由主线程调用 snapshot adapter 时再解释。
        # 这样协议层不需要导入 snapshots 包，也不会提前触碰 runtime 状态。
        return InteractiveMotionCommand(
            kind="set_snapshot",
            command_id=_optional_str(message, "id", label="set_snapshot.id"),
            snapshot=snapshot,
            snapshot_label_map=_optional_string_mapping(
                message,
                "label_map",
                label="set_snapshot.label_map",
            ),
            snapshot_strict=_optional_bool(
                message,
                "strict",
                default=True,
                label="set_snapshot.strict",
            ),
        )
    if command_type == "status":
        _reject_unknown_fields(message, {"type", "id"}, "status")
        return InteractiveMotionCommand(
            kind="status",
            status_id=_optional_str(message, "id", label="status.id"),
        )
    if command_type == "cancel":
        _reject_unknown_fields(message, {"type", "id"}, "cancel")
        cancel_id = _optional_str(message, "id", label="cancel.id")
    ...
    if command_type in {"cancel_current", "estop", "quit"}:
        _reject_unknown_fields(message, {"type"}, command_type)
...
def _segment_allowed_fields(kind: str) -> set[str]:
    """返回一个 canonical Single Scene segment kind 唯一允许消费的字段集合。"""
...
def _optional_string_mapping(
    ...
    *,
    ...
    """读取可选 JSON string-to-string mapping，不转换键或值。"""
    ...
    for item_key, item in value.items():
        ...
        if not isinstance(item, str) or not item:
            raise ValueError(f"{label}[{item_key!r}] must be a non-empty string")
...
def _optional_vector4(
    ...
    *,
    ...
    if key not in message:
        return None
    return np.asarray(_vector(message, key, 4, label=label), dtype=float)
