...
from linkerbot_sim.app.runtime.collision.urdf_kinematics import _UrdfKinematics
...
class RobotObstacleProvider:
    ...
    def from_robot_profile(
        ...
        *,
        ...
        robot_profile = profile.get("robot")
        if not isinstance(robot_profile, Mapping):
            raise ValueError("robot profile requires robot mapping")
        ...
        if config_path is None or urdf_path is None:
            return None
        config = load_yaml(repo_path(str(config_path)))
...
def _root_pose_matrix(pose: RootPoseConfig) -> np.ndarray:
    """把 robot root 的 xyz/rpy 配置转换成 world transform。"""
