...
def parse_args() -> argparse.Namespace:
    ...
    这些参数只控制“使用哪套 profile 和 runtime 运行方式”。本脚本只执行脚本内写定动作；
    实时交互 JSON runtime 由 ``scripts/dual_arm_interactive.py`` 提供。
    """
...
def specified_dual_arm_moves() -> tuple[MoveSpec, ...]:
    """返回脚本内置的双臂指定动作序列。"""
    ...
    return (
        ...
        #         segments=(
        #             TcpArcSegment(
        #                 target_offset=(0.1, 0.1, 0.08),
        #                 intermediate_offset=(0.0, 0.06, 0.05),
...
def run_specified_motion_mode(
    ...
    *,
    ...
    返回 ``(steps, success)``；规划失败属于可报告结果，不在这里抛异常。
    """

    result = run_dual_arm_cumotion_motion_result(
        runtime,
        tcp=tcp,
        moves=specified_dual_arm_moves(),
...
def main() -> None:
    ...
    completed = False
    try:
        steps, success = run_specified_motion_mode(
            runtime,
            args=args,
            tcp=tcp,
        )
        ...
        print(
            f"DUAL_ARM_MOTION_TEST_OK steps={steps} cumotion=specified_moves",
            flush=True,
        )
        completed = True
    finally:
